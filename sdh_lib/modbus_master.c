...
uint8_t ModbusMaster_readCoils(uint8_t SlaveID,uint16_t u16ReadAddress, uint16_t u16BitQty, uint8_t *buf, int buf_size)
{
  ...
  return ModbusMaster_ModbusMasterTransaction(ku8MBReadCoils, buf, buf_size);
}
  ...
  uint16_t u16BitQty, uint8_t *buf, int buf_size)
{
  ...
  _u16ReadAddress = u16ReadAddress;
  _u16ReadQty = u16BitQty;
  return ModbusMaster_ModbusMasterTransaction(ku8MBReadDiscreteInputs, buf, buf_size);
}
  ...
  uint16_t u16ReadQty, uint8_t *buf, int buf_size)
{
  ...
  return ModbusMaster_ModbusMasterTransaction(ku8MBReadHoldingRegisters, buf, buf_size);
}
  ...
  uint8_t u16ReadQty, uint8_t *buf, int buf_size)
{
  ...
  _u16ReadAddress = u16ReadAddress;
  _u16ReadQty = u16ReadQty;
  return ModbusMaster_ModbusMasterTransaction(ku8MBReadInputRegisters, buf, buf_size);
}
...
uint8_t ModbusMaster_writeSingleCoil(uint8_t SlaveID,uint16_t u16WriteAddress, uint8_t u8State, uint8_t *buf, int buf_size)
{
  ...
  return ModbusMaster_ModbusMasterTransaction(ku8MBWriteSingleCoil, buf, buf_size);
}
  ...
  uint16_t u16WriteValue, uint8_t *buf, int buf_size)
{
  ...
  return ModbusMaster_ModbusMasterTransaction(ku8MBWriteSingleRegister, buf, buf_size);
}
  ...
  uint16_t u16BitQty, uint8_t *buf, int buf_size)
{
  ...
  return ModbusMaster_ModbusMasterTransaction(ku8MBWriteMultipleCoils, buf, buf_size);
}
/*
  ...
  uint16_t u16WriteQty, uint8_t *buf, int buf_size)
{
  ...
  return ModbusMaster_ModbusMasterTransaction(ku8MBWriteMultipleRegisters, buf, buf_size);
}
...
uint8_t ModbusMaster_maskWriteRegister(uint8_t SlaveID,uint16_t u16WriteAddress,
  uint16_t u16AndMask, uint16_t u16OrMask, uint8_t *buf, int buf_size)
{
	uint16_t *p_u16 = (uint16_t *)buf;
	_u8MBSlave = SlaveID;
  _u16WriteAddress = u16WriteAddress;
  p_u16[0] = u16AndMask;
  p_u16[1] = u16OrMask;
  return ModbusMaster_ModbusMasterTransaction(ku8MBMaskWriteRegister, buf, buf_size);
}
...
//  uint8_t u8MBStatus = ku8MBSuccess;
	...
	switch(u8MBFunction)
  {
    ...
    case ku8MBWriteSingleRegister:
      u8ModbusADU[u8ModbusADUSize++] = highByte(p_u16[0]);
      u8ModbusADU[u8ModbusADUSize++] = lowByte(p_u16[0]);
      break;
    ...
    case ku8MBWriteMultipleCoils:
      ...
      for (i = 0; i < u8Qty; i++)
      {
        ...
        {
          case 0: // i is even
            u8ModbusADU[u8ModbusADUSize++] = lowByte(p_u16[i >> 1]);
            break;
            
          case 1: // i is odd
            u8ModbusADU[u8ModbusADUSize++] = highByte(p_u16[i >> 1]);
            break;
        }
      }
      break;
    ...
    case ku8MBMaskWriteRegister:
      u8ModbusADU[u8ModbusADUSize++] = highByte(p_u16[0]);
      u8ModbusADU[u8ModbusADUSize++] = lowByte(p_u16[0]);
      u8ModbusADU[u8ModbusADUSize++] = highByte(p_u16[1]);
      u8ModbusADU[u8ModbusADUSize++] = lowByte(p_u16[1]);
      break;
  }
