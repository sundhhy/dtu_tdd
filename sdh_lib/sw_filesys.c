	...
	while(1)
	{
		switch( step)
		{
			...
			case 2:
				...
				if( sup_head->ver[0] != 'V')			//µÚÒ»´ÎÉÏµç£¬²Á³ıÕû¿éflash²¢Ğ´ÈëÍ·ĞÅÏ¢
				{
					...
					if( strcmp(file_in_storage->name, name) == 0x00 )	
					{
						...
						for( j = 0; j < file_in_storage->area_total ;)
						{
							...
							if( end > StrgInfo.sector_size)
							{
								...
								//µ±²éÑ¯µÄÎ»ÖÃ³¬¹ıÉÈÇøµÄ´óĞ¡£¬»¹Ã»ÓĞÕÒµ½ÎÄ¼şµÄ´æ´¢Çø¾ÍÈÏÎªÎÄ¼şÊÇ´íÎóµÄ
...
int fs_write( sdhFile *fd, uint8_t *data, int len)
{
	...
	while( 1)
	{
		...
		if( wr_area->pg_number + wr_area->start_pg > ( wr_sector + 1 ) *  SECTOR_HAS_PAGES )				//ÎÄ¼şµÄ½áÊøÎ»ÖÃ³¬³ö±¾ÉÈÇø£¬ÒÔÉÈÇøµÄ´óĞ¡ÎªÉÏÏŞ
				limit  = SECTOR_SIZE;
...
static int page_malloc( area_t *area, int size)
{
	...
	while(1)
	{
		...
		//todo : ÕâÀïÓĞ¸öbug£¬ÄÚ´æÒ³¹ÜÀíÉÈÇø¹ÜÀíµÄÄÚ´æÒ³Ê±»¥Ïà¶ÀÁ¢µÄ£¬²»ÄÜ×éºÏ³ÉÒ»¸ö×ÜÌå¡£ÀàËÆÓÚwindowµÄDÅÌ²»×ãÊ±ÎŞ·¨´ÓEÅÌ½èÓÃÄÚ´æÒ³
		//Õâ¸öbug¾ö¶¨ÁËÒ»¸öÎÄ¼ş×î´óµÄ³¤¶ÈÊÇÒ»¸öÉÈÇø¹ÜÀíµÄÄÚ´æÒ³µÄ×Ü´óĞ¡¡£³¬¹ıËü¾ÍÎŞ·¨·ÖÅäÄÚ´æÒ³ÁË¡£
	...
	//²Á³ıÊÇÎªÁË°Ñ0Ğ´³É1£¬ÔÚ±ê¼ÇÒÑ¾­±»·ÖÅäµôµÄÒ³Ê±ÊÇÒª°Ñ¶ÔÓ¦µÄbit´Ó1->0
	//Òò´Ë²»»á³öÏÖ0->1µÄ³¡¾°£¬ÎªÁË½ÚÊ¡²Ù×÷¾Í²»½øĞĞ²Á³ı²Ù×÷ÁË¡£
	//Ö»ĞèÒª½«±»·ÖÅäµôµÄÒ³µÄ±êÖ¾Çå³ı¼´¿É
	
	
	begin_pg =area->start_pg % StrgInfo.sector_size * 8;  //Ïû³ıÉÈÇø2µÄÆ«ÒÆ
	for( j = 0; j <= size / StrgInfo.page_size; j ++)
		clear_bit( Flash_buf, begin_pg + j);
	
	//¼ÆËãbegin_pgÕâ¸öÆğÊ¼Î»ÖÃÔÚÒ»¸öÒ³ÖĞµÄÆğÊ¼Î»ÖÃ
	j = begin_pg / 8 % StrgInfo.page_size;
	k = size/begin_pg/8;  //¼ÆËãÒª·ÖÅäµÄ³¤¶ÈËùÕ¼ÓÃµÄ×Ö½Ú³¤¶È
